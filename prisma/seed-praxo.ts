/**
 * Seeds the first Praxo course: "Run Your First Ad Campaign".
 *
 * Usage: npx tsx prisma/seed-praxo.ts
 * Embeddings are generated when OPENAI_API_KEY is set; otherwise chunks are
 * seeded without embeddings and kb search falls back to zero scores until
 * you re-run with a key.
 */
...
const survey = {
  intro: "Five quick questions so your teacher can build a plan around what you're actually promoting.",
  questions: [
    {
      id: 'product',
...
const planTemplate = {
  ...
  steps: [
    ...
    },
    {
      ...
      baseInstructions:
        'Install the tracking pixel/tag on your site and fire a test event.',
...
// Starter knowledge base. Each chunk is written to be read aloud in pieces by
// the voice teacher. Grow this to 40-60 chunks before first cohort (M0 exit).
const kbChunks: { title: string; content: string; stepTags: string[] }[] = [
  {
...
async function main() {
  ...
  const course = await prisma.praxoCourse.upsert({
    ...
    update: { survey, planTemplate, status: 'PUBLISHED' },
  })
  ...
  for (const chunk of kbChunks) {
    ...
    await prisma.praxoKbChunk.create({
      data: {
        ...
        stepTags: chunk.stepTags,
        embedding,
      },
    })
  }

  console.log(`✅ Seeded course "${course.title}" with ${kbChunks.length} KB chunks.`)
}
